...
class ViewCycler:
    ...
    def get_current_view(self) -> str:
        ...
        expired_patterns = [
            pattern
            for pattern, (_, until) in self.context_overrides.items()
            if time.time() >= until
        ]
    ...
    def add_context_override(
        self, pattern: str, view_name: str, duration_seconds: float = 15.0
    ) -> None:
...
class Renderer:
    ...
    def __init__(
        self, views: Dict[str, Dict[str, Any]], view_cycler: ViewCycler, render_timeout_ms: int = 50
    ):
        self.views = views
        ...
        if view_name is None:
            ...
            if context:
                override_view = self.view_cycler.check_context_override(
                    context.get("command", ""), context.get("job", "")
                )
                if override_view:
    ...
    def _render_template(self, template: str, plugin_data: Dict[str, Dict[str, Any]]) -> str:
